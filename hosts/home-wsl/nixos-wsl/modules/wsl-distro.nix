...
with lib; {
  options.wsl = with types;
    let coercedToStr = coercedTo (oneOf [ bool path int ]) (toString) str;
    in {
      ...
      startMenuLaunchers = mkEnableOption
        "shortcuts for GUI applications in the windows start menu";
      wslConf = mkOption {
  ...
  in mkIf cfg.enable {
    ...
    environment.noXlibs = lib.mkForce
      false; # override xlibs not being installed (due to isContainer) to enable the use of GUI apps
    ...
    security.sudo = {
      ...
      '';
      wheelNeedsPassword =
        mkDefault false; # The default user will not have a password by default
    };

    system.activationScripts = {
      copy-launchers = mkIf cfg.startMenuLaunchers (stringAfter [ ] ''
    ...
    warnings = (optional (config.systemd.services.systemd-resolved.enable
