...
with lib; {
  ...
  config = mkIf config.wsl.enable (let
    mkTarball = pkgs.callPackage
      "${lib.cleanSource pkgs.path}/nixos/lib/make-system-tarball.nix";
    ...
    rootfs = let tarball = config.system.build.tarball;
    in "${tarball}/tarball/${tarball.fileName}.tar${tarball.extension}";
    ...
    installer = pkgs.writeScript "installer.sh" ''
      ...
      exec ${
        builtins.unsafeDiscardStringContext config.users.users.root.shell
      } "$@"
    '';
  ...
  in {
    ...
    system.build.installer = mkTarball {
      ...
      storeContents = with pkgs; pkgs2storeContents [ installer ];
